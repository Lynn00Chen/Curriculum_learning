...
def randomly_sample_aug_data(aug_data, k, augs=25, generator=None):
    """
    ...
    if generator is not None:
        # Using the random module with a controlled seed
        indices = list(range(len(aug_data) // augs))
...
if __name__ == "__main__":
    ...
    set_seed(args.seed)
    print(f"Using random seed: {args.seed}") if args.verbose else None
    ...
    # shuffle data with controlled randomness
    generator = torch.Generator().manual_seed(args.seed)
    DATA = DATA[torch.randperm(DATA.shape[0], generator=generator)]
