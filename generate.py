...
def load_model(filepath, compile = False):
    """
    Load a MusicTransformer from a saved pytorch state_dict and hparams.
    """
    from model import MusicTransformer
    from hparams import hparams, device
    ...
    if "state_dict" in file:
        # Final model format
        model.load_state_dict(file["state_dict"], strict=False)
    elif "model_state_dict" in file:
        # Checkpoint format
        model.load_state_dict(file["model_state_dict"], strict=False)
    else:
        raise KeyError("Neither 'state_dict' nor 'model_state_dict' found in the saved file")
